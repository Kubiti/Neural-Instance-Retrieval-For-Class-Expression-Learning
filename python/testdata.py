...
def testdata(kb):
    for kb_name in kb:
        ...
        for exp in keys:
            ...
            if (' ' not in exp and '¬' in exp) or ' ' in exp:
                datapoints.append((exp, data[exp]))
        print('comp', len(datapoints))
        ...
        with open(f"./testing_data/test_data_{kb_name}.json", "w") as file:
...
def dataset_gen(kb):
    for kb_name in kb:
        ...
        with open(f'{filepath}{kb_name}/training_data/Data.json', 'r')  as f:
            data = json.load(f)
